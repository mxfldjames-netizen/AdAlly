...
const BrandManager: React.FC = () => {
  ...
  // Helper to determine if a trial request already exists
  const hasTrialRequest = adIdeas.some(idea => idea.trial_request_id);
  ...
  return (
    <div className="space-y-6 pt-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Brand List & Ad Ideas (lg:col-span-1) */}
        <div className="lg:col-span-1 space-y-6"> {/* Added space-y-6 for vertical spacing */}
          ...
          {selectedBrand && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              {/* === START: MOVED FREE TRIAL LOGIC HERE === */}
              {subscription?.tier === 'free' && selectedBrand && (
                !hasTrialRequest ? (
                  <div className="mb-6 p-4 bg-yellow-50 rounded-lg border border-yellow-200 flex items-center justify-between">
                    <p className="text-sm text-yellow-800 font-medium">Get your first video ad for free!</p>
                    <button
                      onClick={() => handleRequestTrialVideo(selectedBrand.id)}
                      className="flex items-center gap-2 bg-gradient-to-r from-amber-400 to-orange-400 text-black px-3 py-1.5 rounded-lg hover:shadow-lg transition-all duration-200 text-sm font-medium"
                    >
              ...
              {/* === END: MOVED FREE TRIAL LOGIC HERE === */}
              ...
              <div className="space-y-3">
                {/* Apply sort: put ideas with trial_request_id (free trial) first */}
                {[...adIdeas].sort((a, b) => {
                  // If 'a' has trial_request_id and 'b' does not, 'a' comes first (-1)
                  if (a.trial_request_id && !b.trial_request_id) {
                    return -1;
                  }
                  // If 'b' has trial_request_id and 'a' does not, 'b' comes first (1)
                  if (!a.trial_request_id && b.trial_request_id) {
                    return 1;
                  }
                  // Otherwise, maintain the order (which is already sorted by creation date from fetch)
                  return 0; 
                }).map((idea) => {
        ...
        {/* Brand Details & Assets (lg:col-span-2) */}
        <div className="lg:col-span-2">
          {selectedBrand ? (
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  {/* Removed the redundant 'Request Free Trial' button from here */}
                </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
              ...
              <p className="text-gray-500">Choose a brand from the list to view and manage its details and assets</p>
            </div>
          )}
        </div>
      </div>
